...
namespace BlackFox.Win32.UninstallInformations
{
    ...
    {
        public static void ExtractEx(
            string fileName,
            List<Icon> largeIcons,
            List<Icon> smallIcons,
            int firstIconIndex,
            int iconCount)
        {
            /*
        ...
        public static List<Icon> ExtractEx(
            string fileName,
            SystemIconSize size,
            int firstIconIndex,
            int iconCount)
            ...
            out int index)
        {
            ...
            {
                throw new ArgumentException("Empty regString", nameof(regString));
            }
        ...
        private static string GetExtensionIconStringFromKeyUsingDefaultIcon(RegistryKey key)
        {
            Debug.Assert(key != null);

            using (RegistryKey defaultIconKey = key.OpenSubKey("DefaultIcon", false))
            {
                return defaultIconKey?.GetValue(null)?.ToString();
            }
        }

        private static string GetExtensionIconStringFromKeyFromClsid(RegistryKey classesRootKey, string clsid)
        {
            Debug.Assert(classesRootKey != null);
            Debug.Assert(clsid != null);

            using (RegistryKey clsidKey = classesRootKey.OpenSubKey("CLSID", false))
            {
                if (clsidKey == null)
                {
                    return null;
                ...
                using (RegistryKey applicationClsidKey = clsidKey.OpenSubKey(clsid, false))
                {
                    if (applicationClsidKey == null)
                    {
                        return null;
                    }
        ...
        private static string GetExtensionIconStringFromKeyUsingClsid(RegistryKey key)
        {
            Debug.Assert(key != null);

            string applicationClsid;
            using (RegistryKey clsidKey = key.OpenSubKey("CLSID", false))
            {
                object value = clsidKey?.GetValue(null);
                if (value == null)
                {
                    return null;
                }
            ...
            {
                string fromNormalClsid = GetExtensionIconStringFromKeyFromClsid(classesRootKey, applicationClsid);
                if (fromNormalClsid != null)
                {
                    return fromNormalClsid;
                }

                using (RegistryKey wow6432ClassesRootKey = classesRootKey.OpenSubKey("Wow6432Node"))
                {
                    if (wow6432ClassesRootKey == null)
                    {
                        return null;
                    }
        ...
        private static string GetExtensionIconStringFromRegistry(string extension)
        {
            ...
            using (RegistryKey classesRootKey = Registry.ClassesRoot)
            {
                using (RegistryKey extensionKey = classesRootKey.OpenSubKey(extension, false))
                {
                    object fileTypeObject = extensionKey?.GetValue(null);
                    if (fileTypeObject == null)
                    {
                        return null;
                    }

                    using (RegistryKey applicationKey = classesRootKey.OpenSubKey(fileTypeObject.ToString(), false))
                    {
                        string result = GetExtensionIconStringFromKeyUsingClsid(applicationKey);
                        if (result == null)
                        {
        ...
        public class IconNotFoundException : Exception
        {
            public IconNotFoundException(string fileName, int index, Exception innerException)
                : base($"Icon with Id = {index} wasn't found in file {fileName}", innerException)
            {
            }
        }
        ...
        public class UnableToExtractIconsException : Exception
        {
            public UnableToExtractIconsException(string fileName, int firstIconIndex, int iconCount)
                : base(
                    string.Format(
                        "Tryed to extract {2} icons starting from the one with id {1} from the \"{0}\" file but failed",
                        fileName,
                        firstIconIndex,
                        iconCount))
            {
